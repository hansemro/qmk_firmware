...
* Holtek HT32F52352 (Cortex-M0+)
    ...
    * 16 KiB SRAM
    * LQFP64
    * USB/UART ISP bootrom accessed by bridging the BOOT1 pin to ground on reset
...
Make example for this keyboard with the ANSI layout (after setting up your build environment):
    ...
    make cm/mk/prolwhite/ansi:default
...
Flashing example for **unlocked** keyboard (with ANSI layout):
    ...
    make cm/mk/prolwhite/ansi:default:flash
...
### with ISP/Pin-Bridging Method

> [!WARNING]
> Keyboard disassembly is required to access the BOOT1 pin.
...
3. Bridge the BOOT1 and ground pins on the back of the PCB together then plug in the keyboard.
...
4. With the BOOT1 pin bridged, flash QMK binary firmware with mass-erase flag (`-m`).

> [!NOTE]
> A reset is performed after mass erase, so keep BOOT1 pin bridged to re-enter ISP.
...
$ ht32-dfu-tool -w -r write -m 0 /path/to/cm_mk_prolwhite_ansi_default.bin
```
...
* **Configure BOOT1 pin and reset**: Bridge `BOOT1` pin to ground (with a tweezer) and plug in the keyboard
