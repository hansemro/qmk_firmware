...
A TKL keyboard with per-key LED lighting.
...
* Holtek HT32F52352 (Cortex-M0+)
    ...
    * 16 KiB SRAM
    * LQFP64
    * USB/UART ISP bootrom accessed by bridging SEL2 header pins on reset
...
3. Bridge the SEL2 pins on the back of the PCB together then plug in the keyboard.
...
4. With the SEL2 pins bridged, flash QMK binary firmware with mass-erase flag (`-m`).

> [!NOTE]
> A reset is performed after mass erase, so keep SEL2 pins bridged to re-enter ISP.
...
* **Configure BOOT1 pin and reset**: Bridge `SEL2` header pins together (with a tweezer) and plug in the keyboard
