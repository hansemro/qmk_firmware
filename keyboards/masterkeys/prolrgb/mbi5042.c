...
static mbi5042_led_t mbi5042_leds[RGB_MATRIX_LED_COUNT];
...
/* Flush a row's RGB values to MBI5042 starting with output channel 16
...
static inline void mbi5042_write_color_row(int row) {
    ...
    for (int i = MATRIX_COLS - 1; i >= 0; i--) {
        ...
        mbi5042_shift_data((mbi5042_leds[index].b & mask) << 8, MBI5042_SHIFT_REG_WIDTH);
        mbi5042_shift_data((mbi5042_leds[index].g & mask) << 8, MBI5042_SHIFT_REG_WIDTH);
        mbi5042_shift_data_instr((mbi5042_leds[index].r & mask) << 8, MBI5042_SHIFT_REG_WIDTH, MBI5042_DATA_LATCH);
    }
...
#    endif
    ...
    /* Set configuration */
    uint16_t mbi5042_config[MBI5042_NUM_CASCADE] = MBI5042_CONFIGURATION;
