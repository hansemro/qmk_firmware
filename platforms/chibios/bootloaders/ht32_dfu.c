...
void enter_bootloader_mode_if_requested(void) {
    if (bootloader_marker_active()) {
        ...
        // Set SBVT[n] to prevent ISP's SYSTICK ISR from jumping back to main flash
        FMC->SBVT[0] = HT32_BOOTLOADER_ADDRESS;
        FMC->SBVT[1] = HT32_BOOTLOADER_ADDRESS + 0x4;
        FMC->SBVT[2] = HT32_BOOTLOADER_ADDRESS + 0x8;
        FMC->SBVT[3] = HT32_BOOTLOADER_ADDRESS + 0xc;
#else // defined(HT32F52342) || defined(HT32F52352)
        // Set MCUDBGCR to prevent ISP's SYSTICK ISR from jumping back to main flash
